...
describe MetaTags::ViewHelper, 'displaying refresh' do
  ...
  it 'displays refresh when "refresh" is used' do
    ...
    subject.display_meta_tags(:site => 'someSite').tap do |meta|
      expect(meta).to include('<meta content="5" http-equiv="refresh" />')
    end
  end
  ...
  it 'should use custom refresh if given' do
    ...
    subject.display_meta_tags(:site => 'someSite').tap do |meta|
      expect(meta).to include(%Q{<meta content="5;url=http://example.com/" http-equiv="refresh" />})
    end
  end

  it 'should display nothing by default' do
    subject.display_meta_tags(:site => 'someSite').tap do |meta|
      expect(meta).to_not include('http-equiv="refresh"')
    end
  end
end
